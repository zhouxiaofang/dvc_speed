...
def add(  # noqa: C901
    ...
    **kwargs: Any,
    ...
    time2 = datetime.now()
    print('success finished create stages, total use time:', time2 - time1)
    print('成功完成阶段1')
    ...
    with warn_link_failures() as link_failures:
        for stage, source in zip(progress_iter(stages), sources):
            if to_remote or to_cache:
                ...
                try:
                    print("step2: success stage.save() start running... \n")
                    t1 = datetime.now()
                    stage.save()
                    t2 = datetime.now()
                    print("step2: success finished stage.save(), total use time:{}  \n".format( t2 - t1))
                    print('成功完成阶段2')
                    if not no_commit:
                        print("step3: success stage.commit() start running...  \n")
                        t3 = datetime.now()
                        stage.commit()
                        t4 = datetime.now()
                        print("step3: success finished stage.commit(), total use time:{}  \n".format( t4 - t3))
                        print('成功完成阶段3')
